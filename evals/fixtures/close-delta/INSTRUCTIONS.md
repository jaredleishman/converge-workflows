...
2. `original-miss-admin-authorization` — `issue_refund_admin` performs no
   authorization check, a distinct blocker family whose non-waivable baseline
   guarantee was present but whose concrete application the plan and Round 1
   both missed. Closure must classify it `ORIGINAL_MISS` and return `REPLAN` or
   `SPLIT` instead of starting a third broad review.
...
From the scratch repository, capture the remediated candidate and finish the
seeded re-Verify transition with the plugin under test:

```text
python3 <converge-repo>/plugins/converge/scripts/state_gate.py candidate capture-worktree
python3 <converge-repo>/plugins/converge/scripts/state_gate.py set-status READY_FOR_CLOSURE --stage verify
```
...
```bash
python3 evals/grade_review.py evals/fixtures/close-delta "$SCRATCH/.converge/closure.md" --state "$SCRATCH/.converge/state.yaml"
```

Exit code 0 means every expected classification marker and finite state outcome
appeared. The grader
